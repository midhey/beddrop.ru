...
- PHP `^8.2`
- Laravel `^12.0`
- JWT access token + серверные refresh sessions
...
- `Auth` - регистрация, логин, refresh rotation, logout, logout-all
...
- auth session flow для web и mobile клиентов
...
- `/auth` - регистрация, логин, refresh, logout, logout-all
...
- `/courier` - профиль курьера, смены, заказы доставки, статистика заработка
...
- refresh token хранится на backend в виде hash в таблице `auth_refresh_tokens`
- `web`-клиенты получают refresh token через `HttpOnly` cookie
- `mobile`-клиенты получают refresh token в JSON-контракте и хранят его на своей стороне
...
4. Клиент использует refresh token для перевыпуска access token через `/api/v1/auth/refresh`
5. На каждом refresh refresh token ротируется
...
Если backend запускается вне Docker, значения `DB_HOST` и `REDIS_HOST` должны соответствовать локальному окружению. Для Docker Compose используйте `DB_HOST=mysql` и `REDIS_HOST=redis`; для запуска с хоста обычно нужны `127.0.0.1`.
...
- `courier@mail.com` / `courier123`
- `manager@mail.com` / `manager123`
- `staff@mail.com` / `staff123`

Сиды также создают демо-рестораны, категории, товары и активный профиль курьера для `courier@mail.com`.
Самостоятельно созданный профиль курьера считается заявкой: он создаётся со статусом `SUSPENDED` и требует активации администратором.
